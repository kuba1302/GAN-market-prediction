...
if __name__ == "__main__":
    ...
    print(f'---------------------------- TRAIN DATA SHAPE: {data["X_list_train"].shape} ----------------------------')
    ...
    gan = StockTimeGan(generator=generator, discriminator=discriminator, checkpoint_directory=save_path)
    train_history = gan.train(data['X_list_train'],data['Y_preds_real_list_train'], data['Y_whole_real_list_train'], epochs=200)
    ...
    with open(save_path / f'model_{MODEL_VERSION}_class.pickle', 'wb') as handle:
        dill.dump(gan, handle, protocol=pickle.HIGHEST_PROTOCOL)